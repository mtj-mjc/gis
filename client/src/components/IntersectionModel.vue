      ...
      <md-card-expand>
        ...
        <md-card-expand-content>
          <md-card-content>
            ...
            <br />
            <p
              v-if="selectedAdminUnit.length == 2"
            >{{selectedAdminUnit[0].name}} & {{selectedAdminUnit[1].name}}</p>
          </md-card-content>
        </md-card-expand-content>
      </md-card-expand>
    </md-card>
...
export default {
  ...
  mounted() {
    this.$root.$on("selected", data => {
      if (this.selectedAdminUnit.length == 0) {
        ...
        this.clearModel();
        this.$root.$emit("removeSelection", { except: data });
      }
    });
  },
  data: () => ({
  ...
  methods: {
    showModel: function() {
      axios
        .get(
          "http://localhost:3000/intersectionModel?id1=" +
            this.selectedAdminUnit[0].id +
            "&id2=" +
            this.selectedAdminUnit[1].id
        )
        .then(response => {
